...
def run_supercell_generation(config):
    ...
    strained_output = ase_IO.read(f'{prev_wd}/{name}-strained_relaxed.extxyz', index=':')

    for i, eps in enumerate(config['strain']['eps']):
        ph_yaml = f'{cwd}/{name}-{eps}-phonopy.yaml.xz'

        if restart and os.path.isfile(ph_yaml):
            logger.info(f'[restart] supercell generation for {name}-{eps} already complete at {ph_yaml}; skipping')
            continue

        logger.info(f'Generating phonopy object & supercells with displacements for volumetric strain {eps}')
        atoms = strained_output[i]
        phatoms = aseatoms2phonoatoms(atoms)
        ...
        for j, sc in enumerate(ph.supercells_with_displacements):
            sc_atoms = phonoatoms2aseatoms(sc)
            ase_IO.write(f'{cwd}/{name}-{eps}-supercell-{j}.extxyz', sc_atoms, format='extxyz')
