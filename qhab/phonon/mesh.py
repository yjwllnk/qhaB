...
def run_mesh_computation(config):
    ...
    for i, eps in enumerate(config['strain']['eps']):
        ...
        if config['mesh']['mesh']['run']:
            ph.run_mesh(config['mesh']['mesh']['numbers'])
            ph._mesh.write_hdf5(filename=f'{cwd}/{name}-{eps}-mesh.hdf5')
            ph._mesh.write_yaml(filename=f'{cwd}/{name}-{eps}-mesh.yaml') # for debug
        ...
        if config['mesh']['thermal']['run']:
            ...
            ph.write_yaml_thermal_properties(f'{cwd}/{name}-{eps}-thermal_properties.yaml')
            thermal_plt = ph.plot_thermal_properties()
            thermal_plt.savefig(f'{cwd}/{name}-{eps}-thermal_properties.png')
            thermal_plt.close()

        if config['mesh']['tdos']['run']:
            # no difference with auto_total_dos(mesh=mesh_numbers)
            ph.run_total_dos()
            ph.write_total_dos(f'{cwd}/{name}-{eps}-tdos.dat')

            if config['mesh']['band']['run']:
                ph.auto_band_structure(write_yaml=True, filename=f'{cwd}/{name}-{eps}-tdos.yaml')
                band_dos_plt = ph.plot_band_structure_and_dos()
                band_dos_plt.savefig(f'{cwd}/{name}-{eps}-band_dos.png')
                band_dos_plt.close()
